export default {
  async fetch(request, env, ctx) {
    ...
    // 4. Susun Base URL TikWM murni
    const tikwmUrl = `https://www.tikwm.com/api/?url=${encodeURIComponent(targetUrl)}&hd=${hdParam}`;

    // 5. PIPELINE PROXY KEROYOKAN (API di dalam API)
    // Karena dijalankan di Cloudflare, rute pertama langsung tembak TikWM asli (tanpa proxy gratisan)
    const pipelines = [
      tikwmUrl, // Jalur utama: langsung tanpa proxy luar
      `https://api.allorigins.win/raw?url=${encodeURIComponent(tikwmUrl)}`,
      `https://corsproxy.io/?url=${encodeURIComponent(tikwmUrl)}`,
      `https://proxy.corsfix.com/?${encodeURIComponent(tikwmUrl)}`,
    ...
    for (let i = 0; i < pipelines.length; i++) {
      ...
      try {
        ...
        // Validasi struktur response khas TikWM
    ...
    return new Response(JSON.stringify({
      ...
      error_analysis: "Workers gagal menembus Direct Fetch maupun backup multi-proxy pipeline."
    }), {
      status: 502,
      headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
    });
